...
chmod +x ./jq
sudo cp jq /usr/bin
curl --request POST 'https://api.github.com/repos/havellaneda-ar/gh-test/actions/runners/registration-token' --header "Authorization: token ${PERSONAL_ACCESS_TOKEN}" > output.txt
...
~/actions-runner/config.sh --url https://github.com/havellaneda-ar/gh-test/ --token \$runner_token --name "Github EC2 Runner" --unattended
